...
class ResNetSwinFPN(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        posembedh, posembedw = int(spawnh / 4), int(spawnw / 4)
        ...
        for blk in self.encoder.blocks[0:4]:
            x2, (H, W) = blk(x2, H, W)
        x2_spatial = x2.transpose(1, 2)
        B, C, L = x2_spatial.shape
        x2_spatial = x2_spatial.reshape(B, C, H, W)

        x3, (H, W) = self.encoder.blocks[4](x2, H, W)
        for blk in self.encoder.blocks[5:int(4 + self.pvt_depth)]:
            x3, (H, W) = blk(x3, H, W)
        ...
        x2_out = self.layer2_outconv(x2_spatial)
