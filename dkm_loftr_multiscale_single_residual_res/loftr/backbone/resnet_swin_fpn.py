...
from dkm_loftr_multiscale_single_residual_res.models.simmim_rect import SimMIMPVT
...
class ResNetSwinFPN(nn.Module):
    ...
    def __init__(self, config_resfpn, config_simmim):
        ...
        self.layer3 = self._make_layer(block, block_dims[2], stride=1, in_planes=block_dims[2])  # 1/8
    ...
    def _make_layer(self, block, dim, stride=1, in_planes=None):
        if in_planes is None:
            in_planes = self.in_planes
        else:
            in_planes = in_planes
        layer1 = block(in_planes, dim, stride=stride)
