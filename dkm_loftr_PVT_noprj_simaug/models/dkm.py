...
class Decoder(nn.Module):
    ...
    def forward(self, img, mask, f_pyramid, loftr_loss):
        ...
        for new_scale in self.scales:
            ...
            rgb_recon = rgb_recon.detach() + self.conv_refiner[new_scale](feature)
            ...
            loss2_enable = (torch.sum(mask, dim=[1, 2, 3], keepdim=True) > 0).type_as(loss1)

            loss2 = (loss_recon * (1 - mask) * loss2_enable).sum() / (((1 - mask) * loss2_enable).sum() + 1e-5) / in_chans
            losses[new_scale] = loss1 + loss2 * 0.05

            totloss += losses[new_scale]
        totloss = totloss / len(self.scales)

        losses['loftr_loss'] = loftr_loss
        totloss += loftr_loss
...
class RegressionMatcher(nn.Module):
    ...
    def forward(self, data):
        ...
        data['image0'] = data['image0_color']
        data['image1'] = data['image1_color']

        f_q_pyramid, f_s_pyramid, loftr_loss = self.loftr(data)

        f_pyramid = {
            8: torch.cat([f_q_pyramid[8], f_s_pyramid[8]], dim=0),
            4: torch.cat([f_q_pyramid[4], f_s_pyramid[4]], dim=0)
        }

        img = torch.cat([img1, img2], dim=0)
        mask = torch.cat([mask1, mask2], dim=0)

        rgb_recons, losses, totloss = self.decoder(img, mask, f_pyramid, loftr_loss)

        rgb1_scale8, rgb2_scale8 = torch.split(rgb_recons[8], int(rgb_recons[8].shape[0] / 2), dim=0)
        rgb1_scale4, rgb2_scale4 = torch.split(rgb_recons[4], int(rgb_recons[8].shape[0] / 2), dim=0)

        rgb1_recons = {8: rgb1_scale8,
                       4: rgb1_scale4}

        rgb2_recons = {8: rgb2_scale8,
                       4: rgb2_scale4}

        return rgb1_recons, rgb2_recons, losses, totloss