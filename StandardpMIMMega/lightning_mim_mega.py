...
from StandardpMIMMega.models.build_model import DKMv2
from StandardpMIMMega.datasets.megadepth import build_loader_mega
...
def main(config):
    megadepth = build_loader_mega(config, logger)
    logger.info(f"The Length of ScanNet is %d {megadepth.__len__()}")
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        megadepth_sampler = DistributedSampler(megadepth, num_replicas=dist.get_world_size(), rank=dist.get_rank(), shuffle=True)
        megadepth_sampler.set_epoch(int(epoch))
        data_loader_train_megadepth = DataLoader(
            megadepth, config.DATA.BATCH_SIZE, sampler=megadepth_sampler,
            num_workers=config.DATA.NUM_WORKERS, pin_memory=True, drop_last=True, collate_fn=collate_fn)
        data_loader_train_megadepth = iter(data_loader_train_megadepth)

        train_one_epoch(config, model, data_loader_train_megadepth, optimizer, epoch, lr_scheduler, writer)
...
def train_one_epoch(config, model, data_loader, optimizer, epoch, lr_scheduler, writer):
    ...
    for idx in range(num_steps):
        mega_batch = next(data_loader)

        img1_mega, mask_mega, img2_mega, _ = mega_batch

        img1_mega = img1_mega.cuda(non_blocking=True)
        img2_mega = img2_mega.cuda(non_blocking=True)
        mask_mega = mask_mega.cuda(non_blocking=True)

        loss, x_rec = model(img1_mega, mask_mega, img2_mega)
        ...
        if (idx % config.PRINT_FREQ == 0) and (writer is not None):
            img_vls = img * torch.from_numpy(np.array(IMAGENET_DEFAULT_STD)).view(
                ...
                [1, 3, 1, 1]).cuda().float()
            img2_vls = img2_mega * torch.from_numpy(np.array(IMAGENET_DEFAULT_STD)).view(
                [1, 3, 1, 1]).cuda().float() + torch.from_numpy(np.array(IMAGENET_DEFAULT_MEAN)).view(
                [1, 3, 1, 1]).cuda().float()
